...
import config
from core.handlers.basic import cmd_start, get_day_card, get_layout, admin_panel, Layout, get_horoscope, \
    edit_card_desc, layout_context, layout_image, layout_title
from core.handlers.callback import select_card_of_day, change_layout, layout_call, \
...
from core.handlers.basic_moderation import moder_get_day_card, moder_get_horoscope, moder_get_layout
...
async def start_aiogram_bot(bot: Bot):
    await set_command(bot)
    dp = Dispatcher()
    ...
    dp.message.register(moder_get_day_card, Command(commands=['moder_card']))
    dp.message.register(moder_get_layout, Command(commands=['moder_layout']))
    dp.message.register(moder_get_horoscope, Command(commands=['moder_horoscope']))
...
async def main():
    # Создаем бот Aiogram
    aiogram_bot = Bot(token=config.TOKEN)

    # Создаем задачи для запуска ботов Aiogram и Pyrogram
    await asyncio.gather(
        start_aiogram_bot(aiogram_bot),
        start_pyrogram_bots()
    )
